...
export default function Topbar({ callActive, onAddLead, onImport }) {
  ...
  return (
    <div style={{
      ...
      padding: '0 16px', height: 48, flexShrink: 0, zIndex: 200,
    }}>
      ...
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 7 }}>
        {callActive && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontFamily: 'var(--mono)',
            fontSize: 10, color: 'var(--red)', background: 'var(--rbg)',
            border: '1px solid var(--rdim)', borderRadius: 4, padding: '3px 10px' }}>
            <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--red)',
              animation: 'pulse 1.2s infinite', display: 'inline-block' }} />
            LIVE — {callLead?.name}
          </div>
        )}
        <button onClick={onImport} style={{ height: 30, padding: '0 10px', borderRadius: 4,
        ...
        <button onClick={onAddLead} style={{ height: 30, padding: '0 12px', borderRadius: 4,
