...
export default function PostCallModal({ callData, consecutiveNoAnswers, onClose }) {
  ...
  const isOutsideWorkingHours = useMemo(() => {
    ...
    const [hour, min] = nextTime.split(':').map(Number);
    const totalMins = hour * 60 + min;
    return totalMins < 10 * 60 || totalMins > 19 * 60;
  }, [nextTime]);
  ...
  const handleSave = async () => {
    ...
    const followUp    = nextDate ? new Date(`${nextDate}T${nextTime || '10:00'}`) : null;
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            {isOutsideWorkingHours && (
              <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--amber)', marginTop: 5 }}>
                ⚠ Warning: Scheduled outside working hours (10:00 AM - 7:00 PM)
              </div>
            )}
          </div>
