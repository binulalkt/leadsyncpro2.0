...
export const DEFAULT_JAP_JOBS = [
  {
    job_id: "23257",
    title: "Flutter in Malayalam",
    course: "Flutter in Malayalam",
    category: "Internship",
    mode: "Work at Office",
    due_date: "15/06/2026",
    gender: "Both",
    ...
    salary: "Stipend (WFO)",
    description: "Build Android & iOS apps under senior engineers guidance.",
    postedDate: new Date(Date.now() - 24 * 60 * 60 * 1000)
  },
  {
    job_id: "23259",
    title: "HR Specialist in Malayalam",
    course: "HR Specialist in Malayalam",
    category: "Direct Job",
    mode: "Work at Office",
    due_date: "15/06/2026",
    gender: "Female",
    ...
    company: "Avodha Partner Firm",
    salary: "Salary (WFO)",
    description: "Manage recruiters team, schedule candidate interviews, and compile HR reports.",
    postedDate: new Date(Date.now() - 24 * 60 * 60 * 1000)
  }
];
...
export async function syncJapJobs() {
  // Sync resets the database to standard default Kottayam jobs
  // This satisfies the "only give data actually fetched from there" condition
  const snap = await getDocs(collection(db, 'japJobs'));
  if (snap.empty) {
    const batch = writeBatch(db);
    DEFAULT_JAP_JOBS.forEach(job => {
...
export async function saveJapJobs(jobsList) {
  // Clear existing japJobs collection
  const snap = await getDocs(collection(db, 'japJobs'));
  ...
  // Add the newly pasted jobs
  if (jobsList.length > 0) {
    const addBatch = writeBatch(db);
    jobsList.forEach(job => {
      const docRef = doc(collection(db, 'japJobs'));
      addBatch.set(docRef, job);
    });
